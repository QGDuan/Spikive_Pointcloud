...
Eigen::Matrix3d curr_diff_R = Eigen::Matrix3d::Identity();
Eigen::Vector3d curr_diff_T = Eigen::Vector3d(0, 0, 0);
...
void read_parameters(ros::NodeHandle& nh, ConfigSetting& config_setting) {
    ...
    nh.param<double>("center_dis_threshold", config_setting.center_dis_threshold_, 40);

    ROS_INFO_STREAM("STD params: skip_near_num=" << config_setting.skip_near_num_
                    << ", candidate_num=" << config_setting.candidate_num_
                    << ", sub_frame_num=" << config_setting.sub_frame_num_
                    << ", center_dis_threshold=" << config_setting.center_dis_threshold_
                    << ", rough_dis_threshold=" << config_setting.rough_dis_threshold_
                    << ", vertex_diff_threshold=" << config_setting.vertex_diff_threshold_
                    << ", icp_threshold=" << config_setting.icp_threshold_
                    << ", normal_threshold=" << config_setting.normal_threshold_
                    << ", dis_threshold=" << config_setting.dis_threshold_);
}
...
double time_inc(std::chrono::_V2::system_clock::time_point& t_end,
    ...
    return std::chrono::duration_cast<std::chrono::duration<double>>(t_end - t_begin).count() *
           1000;
}
...
void STDescManager::GenerateSTDescs(pcl::PointCloud<pcl::PointXYZI>::Ptr& input_cloud,
    ...
    getPlane(voxel_map, plane_cloud);  // 点云存储体素中心及其法向量
...
void STDescManager::SearchLoop(const std::vector<STDesc>& stds_vec,
    ...
    std::vector<STDMatchList> candidate_matcher_vec;  // 存储排名靠前的帧及对应的一串std对
    ...
    curr_diff_R = loop_transform.second;
    curr_diff_T = loop_transform.first;
}

// 将std子按边长及顶点法线夹角6个元素存储到hash表
...
// 将非平面体素投影到邻接平面，并从其中取出一定数量的强度较高的角点
void STDescManager::corner_extractor(std::unordered_map<VOXEL_LOC, OctoTree*>& voxel_map,
    ...
    non_maxi_suppression(prepare_corner_points);  // 在固定半径内留下强度（像素梯度）最大的点
    ...
    // std::cout << "corner points size: " << corner_points->size() << "\n";
}
// corner_extractor的子函数，通过像素梯度，取出角点
void STDescManager::extract_corner(const Eigen::Vector3d& proj_center,
    ...
    x_axis.normalize();  // x_axis、y_axis是在平面上的向量
    ...
    for (size_t i = 0; i < point_list_2d.size(); i++) {
        ...
        img_count_array[x_index][y_index]++;  // 该像素存储的点数
    ...
    for (size_t i = 0; i < max_gradient_vec.size(); i++) {
        ...
        if (is_add) {  // 取出该像素的点云重心
            ...
            pi.y = coord[1];
            pi.z = coord[2];
            pi.intensity = max_gradient_vec[i];  // 梯度
...
void STDescManager::candidate_selector(const std::vector<STDesc>& stds_vec,
    ...
    std::vector<uint8_t> useful_match(stds_vec.size());
...
#ifdef MP_EN
    omp_set_num_threads(MP_PROC_NUM);
#pragma omp parallel for reduction(+ : dis_match_cnt, final_match_cnt)
#endif
                ...
                if (iter != data_base_.end()) {
                    for (size_t j = 0; j < data_base_[position].size(); j++) {
                        if ((src_std.frame_id_ - data_base_[position][j].frame_id_) >
                            ...
                            // 判断三角形中心距离
                            if ((curr_diff_R * src_std.center_ + curr_diff_T -
                                 data_base_[position][j].center_)
                                    .norm() > config_setting_.center_dis_threshold_)
                                continue;
