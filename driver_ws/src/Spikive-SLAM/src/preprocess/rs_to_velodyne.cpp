...
// rs相对avia的位姿
std::vector<double> rta_T_vct(3, 0.0);
std::vector<double> rta_E_vct(3, 0.0);
std::vector<double> rta_R_vct(9, 0.0);
Eigen::Vector3d rta_T(0, 0, 0);
Eigen::Vector3d rta_E(0, 0, 0);
Eigen::Matrix3d rta_R(Eigen::Matrix3d::Identity());
...
void rsHandler_lsdc(sensor_msgs::PointCloud2 pc_msg) {
    ...
    for (int point_id = 0; point_id < pt_size; ++point_id) {
        ...
        // new_point.time = float(count_point * 0.1 / pt_size);
...
int main(int argc, char** argv) {
    ...
    // 读取rs相对于avia位姿
    nh.param<std::vector<double>>("mapping/rs_to_avia_T", rta_T_vct,
                                  std::vector<double>({0, 0, 0}));
    nh.param<std::vector<double>>("mapping/rs_to_avia_E", rta_E_vct,
                                  std::vector<double>({0, 0, 0}));
    nh.param<std::vector<double>>("mapping/rs_to_avia_R", rta_R_vct,
                                  std::vector<double>({1, 0, 0, 0, 1, 0, 0, 0, 1}));
    rta_T << rta_T_vct[0], rta_T_vct[1], rta_T_vct[2];
    rta_R << rta_R_vct[0], rta_R_vct[1], rta_R_vct[2], rta_R_vct[3], rta_R_vct[4], rta_R_vct[5],
        rta_R_vct[6], rta_R_vct[7], rta_R_vct[8];
    rta_E << rta_E_vct[0] * M_PI / 180, rta_E_vct[1] * M_PI / 180, rta_E_vct[2] * M_PI / 180;
    if (rta_E(0) != 0 || rta_E(1) != 0 || rta_E(2) != 0)
        rta_R = (Eigen::AngleAxisd(rta_E(0), Eigen::Vector3d::UnitZ()) *
                 Eigen::AngleAxisd(rta_E(1), Eigen::Vector3d::UnitY()) *
                 Eigen::AngleAxisd(rta_E(2), Eigen::Vector3d::UnitX()))
                    .toRotationMatrix();
    // std::cout << "Extrinsic T and R of rs to avia:\n" << rta_T << "\n" << rta_R << "\n";

    subRobosensePC = nh.subscribe(lidar_topic, 100000, rsHandler_lsdc);
    pubRobosensePC = nh.advertise<sensor_msgs::PointCloud2>("/velodyne_points", 100000);
